
# Masjid Tua-Game

<img width="1452" height="816" alt="Screenshot 2026-06-06 094120" src="https://github.com/user-attachments/assets/5756093f-b47d-41ca-932f-98bba4c0fe9f" />


Masjid Tua-Game is a first-person horror adventure game developed using Unity and C#. Players take the role of a young santri tasked with restoring the sanctity of an abandoned mosque that has been overtaken by jinn and shrouded in darkness.

The game combines exploration, resource management, and survival horror elements. Players must search for scattered Qur'an manuscripts, manage limited flashlight batteries, survive supernatural encounters, and ultimately perform the adzan to cleanse the mosque.

## Screenshots

<img width="1486" height="823" alt="Screenshot 2026-06-06 094404" src="https://github.com/user-attachments/assets/79a82579-d401-4d2d-a077-957ceea97248" />

<img width="1471" height="803" alt="Screenshot 2026-06-06 094546" src="https://github.com/user-attachments/assets/465947ab-689c-4468-898c-e8d87def7e00" />

<img width="1470" height="821" alt="Screenshot 2026-06-06 094615" src="https://github.com/user-attachments/assets/ad845d2d-ad83-41aa-990c-8de87ab6a112" />

<img width="1459" height="809" alt="Screenshot 2026-06-06 094805" src="https://github.com/user-attachments/assets/39436979-3759-49ed-a5ee-601b0b1039d5" />

<img width="1474" height="827" alt="Screenshot 2026-06-06 094905" src="https://github.com/user-attachments/assets/c4561cc0-6b79-4c24-b44f-4ca01f921aac" />

<img width="1466" height="804" alt="Screenshot 2026-06-06 095001" src="https://github.com/user-attachments/assets/08521a96-1eba-42c3-b2da-01de61a5a6ad" />

<img width="1470" height="816" alt="Screenshot 2026-06-06 095023" src="https://github.com/user-attachments/assets/d123128e-0ff2-4a72-8f90-fc717b6e4789" />

---

## Features

- First-Person Horror Experience
- Atmospheric Environment and Lighting
- Labyrinth-Based Exploration
- Qur'an Manuscript Collection System
- Flashlight Battery Management
- Dynamic Jinn Encounters
- Story-Driven Progression
- Multiple Exploration Areas
- Final Adzan Cleansing Sequence

---

## Story

A once-sacred mosque has been abandoned for years and transformed into a dark labyrinth inhabited by jinn. As a young santri, the player is assigned a sacred mission: recover the lost Qur'an manuscripts scattered throughout the mosque and return them to their rightful place.

As the mission progresses, the supernatural presence within the mosque becomes increasingly aggressive. Only after all manuscripts have been restored can the player reach the Imam Room and perform the final adzan to drive away the jinn and restore peace.

---

## Gameplay Objective

The player's objective is to:

- Explore the abandoned mosque
- Locate all lost manuscripts
- Collect flashlight batteries
- Survive encounters with jinn
- Return manuscripts to the main bookshelf
- Reach the Imam Room
- Perform the final adzan

---

## Technologies Used

- Unity
- C#
- EasyRoads3D
- Visual Studio
- Git & GitHub

---

## Project Information

| Category | Information |
|-----------|------------|
| Genre | First-Person Horror Adventure |
| Engine | Unity |
| Programming Language | C# |
| Platform | Windows |
| Status | Completed |

---

## Development Team

This project was developed collaboratively as part of an academic game development project.

Project Information

Genre: First-Person Horror Adventure

Platform: Windows

Engine: Unity

Programming Language: C#

Development Status: Completed
